...
var ExplorerDB = require('./explorerDB.js');

module.exports = class ZombieNetExplorer extends ExplorerDB {
    crawlers = {};
    ...
    async setup_parachain(paraID, chainName, id, rpc_port, para) {
	let nodes = para.nodes;
	let node = nodes[0]; // pick first node
	let chainID = paraID;
	let WSEndpoint = node.wsUri;
	let RPCEndpoint = rpc_port ? `http://127.0.0.1:${rpc_port}` : "";
	let sql = `insert into chain (chainID, chainName, id, WSEndpoint, evmRPC ) values ( '${paraID}', '${chainName}', '${id}', '${WSEndpoint}', '${RPCEndpoint}') on duplicate key update WSEndpoint = values(WSEndpoint), blocksFinalized = 1, blocksCovered = 1`
	this.batchedSQL.push(sql)
	...
	try {
		...
		await this.setup_parachain(paraID, chainName, name, rpc_port, zombie.paras[paraID]);
	    }
